...
public class TransactionService implements ITransactionService {
	...
	public List<Transaction> getTransactions(Account account) {
		...
		List<Integer> relationshipsIds = relationshipRepository.getAllRelationshipsIds(accountId);

		for(Integer relationshipId : relationshipsIds) {
			result = transactionRepository.getTransactions(relationshipId);
			int connectionId = relationshipRepository.getConnectionId(relationshipId);
			Account relationship = accountRepository.getAccount(connectionId);
			for(Transaction transaction : result) {
				transaction.setRelationshipAccount(relationship);
			}
